...
// import HolidayInfo, { place, ItineraryDay, Hotel } from './ChatBox';
...
// const {TripDetailInfo}=useTripDetail() as { TripDetailInfo: HolidayInfo };
...
// Corrected Iternary.tsx component


// "use client";

// import React from 'react';
// import { Timeline } from "@/components/ui/timeline"; // Make sure the path is correct
// import { useTripDetail } from '@/app/provider';
// import PlaceCarditem from './PlaceCarditem';
// import HotelCarditem from './HotelCarditem';
// // import { HolidayInfo, place } from './ChatBox';
// import { HolidayInfo,place } from "@/types/trip";
...
// function Iternary() {
//   //@ts-ignore
//   const { TripDetailInfo } = useTripDetail() as { TripDetailInfo: HolidayInfo | null };
...
//         content: (
//           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//             {/* Defensive check for hotels array */}
//             {TripDetailInfo.hotels?.map((hotel, index) => (
//               <div key={index}>
//                 <HotelCarditem hotel={hotel} />
//               </div>
...
//         content: (
//           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//             {/* Defensive check for places array */}
//             {day.places?.map((place: place, i: React.Key | null | undefined) => (
//               <div key={i} className="border rounded-lg p-4 shadow-sm">
//                 <PlaceCarditem activity={place} />
//               </div>
...
//         ),
//       })),
//     ].filter(item => item !== null); // Filter out any null values if a map returns null
//   }
...
import Image from "next/image";
import WaitingHotel from "../../../public/WaitingHotel.jpg"
...
export default function Iternary() {
  ...
  if (TripDetailInfo) {
    data = [
      ...
      ...TripDetailInfo.itinerary?.map((day, index) => ({
        ...
        content: (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {day.places?.map((activity: Place, i: number) => {
              // Convert geo_coordinates if needed
              ...
              return <PlaceCarditem key={i} activity={mappedActivity} />;
            })}
          </div>
        ),
      })),
  ...
  <Timeline data={data} tripData={TripDetailInfo} />
    ...
    <div>
         <div
    className={`flex flex-col items-center justify-center py-10 text-gray-700 dark:text-gray-300 ${poppins.className}`}
  >
    ...
    <p className="text-sm mt-2 text-gray-500 dark:text-gray-400">
      Sit tight while Holiday Buddy finds the best spots 🌍✈️
    </p>
  </div>
    </div>
)}
